...
def attention_decoder(_hps, 
  ...
  with variable_scope.variable_scope("attention_decoder") as scope:
    ...
    for i, inp in enumerate(emb_dec_inputs):
      ...
      with variable_scope.variable_scope("AttnOutputProjection"):
        output = linear([cell_output] + [context_vector], cell.output_size, True) # (batch_size, hidden_dim)
      ...
      with tf.variable_scope('output_projection'):
        ...
        if FLAGS.share_decoder_weights:
          w_out = tf.transpose(math_ops.tanh(linear([embedding] + [tf.transpose(w_out)], _hps.dec_hidden_dim, bias=False)))
      ...
      if FLAGS.use_discounted_rewards or FLAGS.use_intermediate_rewards:
        ...
        for _ in range(_hps.k):
          rl_fscore = rouge_l_fscore(tf.transpose(tf.stack(samples)[:, :, _]), target_batch[:, :(i + 1)])
          _sampling_rewards.append(tf.reshape(rl_fscore, [-1, 1]))
          rl_fscore = rouge_l_fscore(tf.transpose(tf.stack(greedy_search_samples)[:, :, _]), target_batch[:, :(i + 1)])  # shape (batch_size, 1)
  ...
  # all tensors should be of size (batch_size, ...)
  return (tf.stack(outputs, axis=1), # (batch_size, max_dec_steps, hidden_dim)
          state, # (batch_size, hidden_dim)
          tf.stack(attn_dists, axis=1), # (batch_size, max_dec_steps, attn_dim)
          tf.stack(p_gens, axis=1), # (batch_size, max_dec_steps)
          coverage, # (batch_size, hidden_dim)
          tf.stack(vocab_scores, axis=1), # (batch_size, max_dec_steps, vocab_len)
          tf.stack(final_dists, axis=1), # (batch_size, max_dec_steps, extended_vocab_len)
          tf.stack(samples, axis=1), # (batch_size, max_dec_steps, k)
          tf.stack(greedy_search_samples, axis=1), # (batch_size, max_dec_steps, k)
          tf.stack(temporal_e, axis=1), # (batch_size, max_dec_steps, attn_dim)
          tf.stack(sampling_rewards,
                   axis=1) if (FLAGS.use_discounted_rewards or FLAGS.use_intermediate_rewards) else sampling_rewards,
          # (batch_size, max_dec_steps, k)
          tf.stack(greedy_rewards, axis=1) if (
                    FLAGS.use_discounted_rewards or FLAGS.use_intermediate_rewards) else greedy_rewards
          # (batch_size, max_dec_steps, k)
          )
