...
# Where to find data
tf.app.flags.DEFINE_string('full_data_path', '', 'Path expression to tf.Example datafiles. Can include wildcards to access multiple datafiles.')
tf.app.flags.DEFINE_string('partial_data_path', '', 'Path expression to tf.Example datafiles. Can include wildcards to access multiple datafiles.')
tf.app.flags.DEFINE_boolean('partial_decoding', True, 'Decoding partial dataset or full dataset.')
...
tf.app.flags.DEFINE_string('decode_from', 'train', 'must be one of train/eval')
tf.app.flags.DEFINE_string('decode_name', '', 'unique_name for dataset')
...
tf.app.flags.DEFINE_integer('train_size', 287226, 'Number of training samples')
...
tf.app.flags.DEFINE_integer('eval_interval', 200, 'which gpu to use to train the model')

# Pointer-generator or baseline model
tf.app.flags.DEFINE_boolean('pointer_gen', True, 'If True, use pointer-generator model. If False, use baseline model.')
tf.app.flags.DEFINE_boolean('partial_rewarding', False, 'If True, use partial sentence rewarding.')
tf.app.flags.DEFINE_boolean('rouge_based_eval', False, 'If True, use ROUGE values to select the best model.')
...
tf.app.flags.DEFINE_boolean('avoid_trigrams', False, 'Avoids trigram during decoding')
...
tf.app.flags.DEFINE_boolean('use_discounted_rewards', False, 'Whether to use discounted rewards.') # WARNING: too time-consuming
tf.app.flags.DEFINE_boolean('use_intermediate_rewards', False, 'Whether to use intermediate rewards.') # WARNING: too time-consuming
...
tf.app.flags.DEFINE_float('zeta', 0, 'Only when using fixed_zeta: the RL training Full/Partial RL loss scaling factor, 1 means use only Full RL loss, 0 means use only Partial RL loss')
tf.app.flags.DEFINE_boolean('fixed_zeta', False, 'Use fixed value for zeta or adaptive based on global step')
tf.app.flags.DEFINE_float('zeta_clipping', 0.5, 'Clipping zeta value.')
tf.app.flags.DEFINE_integer('rl_start_step', 0, 'Step where we start RL training')
...
class Seq2Seq(object):
  ...
  def restore_best_eval_model(self):
    ...
    event_files = sorted(
      glob('{}/eval_{}/events*'.format(FLAGS.log_root, "rouge" if FLAGS.rouge_based_eval else "loss")))
    best_loss = None
    best_rouge = None
    best_rouge_step = 0
    best_step = 0
    ...
    for ef in event_files:
      try:
        ...
        print(ex.message)
        continue
    if FLAGS.rouge_based_eval:
      best_loss = best_rouge
      best_step = best_rouge_step
    tf.logging.info(
      'resotring best {} from the current logs: {}\tstep: {}'.format("rouge" if FLAGS.rouge_based_eval else "loss",
                                                                     best_loss,
                                                                     best_step))
    time.sleep(10)
    return best_loss
  ...
  def setup_training(self):
    ...
    if FLAGS.convert_to_reinforce_model:
      assert (
                FLAGS.rl_training or FLAGS.ac_training), "To convert your pointer model to a reinforce model, run with convert_to_reinforce_model=True and either rl_training=True or ac_training=True"
