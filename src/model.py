...
class SummarizationModel(object):
  ...
  '''
  def _add_placeholders_full(self):
    """Add placeholders for full dataset to the graph. These are entry points for any input data."""
    hps = self._hps

    # encoder part
    self._enc_batch_full = tf.placeholder(tf.int32, [hps.batch_size, None], name='enc_batch_full')
    self._enc_lens_full = tf.placeholder(tf.int32, [hps.batch_size], name='enc_lens_full')
    self._enc_padding_mask_full = tf.placeholder(tf.float32, [hps.batch_size, None], name='enc_padding_mask_full')
    if FLAGS.pointer_gen:
      self._enc_batch_extend_vocab_full = tf.placeholder(tf.int32, [hps.batch_size, None], name='enc_batch_extend_vocab_full')
      self._max_art_oovs_full = tf.placeholder(tf.int32, [], name='max_art_oovs_full')

    # decoder part
    self._dec_batch_full = tf.placeholder(tf.int32, [hps.batch_size, hps.max_dec_steps], name='dec_batch_full')
    self._target_batch_full = tf.placeholder(tf.int32, [hps.batch_size, hps.max_dec_steps], name='target_batch_full')
    self._dec_padding_mask_full = tf.placeholder(tf.float32, [hps.batch_size, hps.max_dec_steps], name='dec_padding_mask_full')

    if hps.mode == "decode":
      if hps.coverage:
        self.prev_coverage_full = tf.placeholder(tf.float32, [hps.batch_size, None], name='prev_coverage_full')
      if hps.intradecoder:
        self.prev_decoder_outputs_full = tf.placeholder(tf.float32, [None, hps.batch_size, hps.dec_hidden_dim], name='prev_decoder_outputs_full')
      if hps.use_temporal_attention:
        self.prev_encoder_es_full = tf.placeholder(tf.float32, [None, hps.batch_size, None], name='prev_encoder_es_full')

  def _add_placeholders_partial(self):
    """Add placeholders for partial dataset to the graph. These are entry points for any input data."""
    hps = self._hps

    # encoder part
    self._enc_batch_partial = tf.placeholder(tf.int32, [hps.batch_size, None], name='enc_batch_partial')
    self._enc_lens_partial = tf.placeholder(tf.int32, [hps.batch_size], name='enc_lens_partial')
    self._enc_padding_mask_partial = tf.placeholder(tf.float32, [hps.batch_size, None], name='enc_padding_mask_partial')
    if FLAGS.pointer_gen:
      self._enc_batch_extend_vocab_partial = tf.placeholder(tf.int32, [hps.batch_size, None], name='enc_batch_extend_vocab_partial')
      self._max_art_oovs_partial = tf.placeholder(tf.int32, [], name='max_art_oovs_partial')

    # decoder part
    self._dec_batch_partial = tf.placeholder(tf.int32, [hps.batch_size, hps.max_dec_steps], name='dec_batch_partial')
    self._target_batch_partial = tf.placeholder(tf.int32, [hps.batch_size, hps.max_dec_steps], name='target_batch_partial')
    self._dec_padding_mask_partial = tf.placeholder(tf.float32, [hps.batch_size, hps.max_dec_steps], name='dec_padding_mask_partial')

    if hps.mode == "decode":
      if hps.coverage:
        self.prev_coverage_partial = tf.placeholder(tf.float32, [hps.batch_size, None], name='prev_coverage_partial')
      if hps.intradecoder:
        self.prev_decoder_outputs_partial = tf.placeholder(tf.float32, [None, hps.batch_size, hps.dec_hidden_dim], name='prev_decoder_outputs_partial')
      if hps.use_temporal_attention:
        self.prev_encoder_es_partial = tf.placeholder(tf.float32, [None, hps.batch_size, None], name='prev_encoder_es_partial')

  def _make_feed_dict_full(self, batch, just_enc=False):
    ...
    """
    feed_dict = {}
    feed_dict[self._enc_batch_full] = batch.enc_batch
    feed_dict[self._enc_lens_full] = batch.enc_lens
    feed_dict[self._enc_padding_mask_full] = batch.enc_padding_mask
    if FLAGS.pointer_gen:
      feed_dict[self._enc_batch_extend_vocab_full] = batch.enc_batch_extend_vocab
      feed_dict[self._max_art_oovs_full] = batch.max_art_oovs
    if not just_enc:
      feed_dict[self._dec_batch_full] = batch.dec_batch
      feed_dict[self._target_batch_full] = batch.target_batch
      feed_dict[self._dec_padding_mask_full] = batch.dec_padding_mask
    return feed_dict

  def _make_feed_dict_partial(self, batch, just_enc=False):
    """Make a feed dictionary mapping parts of the batch to the appropriate placeholders.
    ...
    """
    feed_dict = {}
    feed_dict[self._enc_batch_partial] = batch.enc_batch
    feed_dict[self._enc_lens_partial] = batch.enc_lens
    feed_dict[self._enc_padding_mask_partial] = batch.enc_padding_mask
    if FLAGS.pointer_gen:
      feed_dict[self._enc_batch_extend_vocab_partial] = batch.enc_batch_extend_vocab
      feed_dict[self._max_art_oovs_partial] = batch.max_art_oovs
    if not just_enc:
      feed_dict[self._dec_batch_partial] = batch.dec_batch
      feed_dict[self._target_batch_partial] = batch.target_batch
      feed_dict[self._dec_padding_mask_partial] = batch.dec_padding_mask
    return feed_dict
  ...
  '''
  def _add_decoder_full(self, emb_dec_inputs, embedding):
    ...
    prev_coverage = self.prev_coverage_full if (hps.mode=="decode" and hps.coverage) else None # In decode mode, we run attention_decoder one step at a time and so need to pass in the previous step's coverage vector each time
    prev_decoder_outputs = self.prev_decoder_outputs_full if (hps.intradecoder and hps.mode=="decode") else tf.stack([],axis=0)
    prev_encoder_es = self.prev_encoder_es_full if (hps.use_temporal_attention and hps.mode=="decode") else tf.stack([],axis=0)
    return attention_decoder(_hps=hps,
      v_size=self._vocab.size(),
      _max_art_oovs=self._max_art_oovs_full,
      _enc_batch_extend_vocab=self._enc_batch_extend_vocab_full,
      emb_dec_inputs=emb_dec_inputs,
      target_batch=self._target_batch_full,
      _dec_in_state=self._dec_in_state_full,
      _enc_states=self._enc_states_full,
      enc_padding_mask=self._enc_padding_mask_full,
      dec_padding_mask=self._dec_padding_mask_full,
      cell=cell,
      embedding=embedding,
  ...
  def _add_decoder_partial(self, emb_dec_inputs, embedding):
    """Add attention decoder to the graph. In train or eval mode, you call this once to get output on ALL steps. In decode (beam search) mode, you call this once for EACH decoder step.
    ...
    prev_coverage = self.prev_coverage_partial if (hps.mode=="decode" and hps.coverage) else None # In decode mode, we run attention_decoder one step at a time and so need to pass in the previous step's coverage vector each time
    prev_decoder_outputs = self.prev_decoder_outputs_partial if (hps.intradecoder and hps.mode=="decode") else tf.stack([],axis=0)
    prev_encoder_es = self.prev_encoder_es_partial if (hps.use_temporal_attention and hps.mode=="decode") else tf.stack([],axis=0)
    return attention_decoder(_hps=hps,
      v_size=self._vocab.size(),
      _max_art_oovs=self._max_art_oovs_partial,
      _enc_batch_extend_vocab=self._enc_batch_extend_vocab_partial,
      emb_dec_inputs=emb_dec_inputs,
      target_batch=self._target_batch_partial,
      _dec_in_state=self._dec_in_state_partial,
      _enc_states=self._enc_states_partial,
      enc_padding_mask=self._enc_padding_mask_partial,
      dec_padding_mask=self._dec_padding_mask_partial,
      cell=cell,
      embedding=embedding,
      sampling_probability=self._sampling_probability if FLAGS.scheduled_sampling else 0,
      alpha=self._alpha if FLAGS.E2EBackProp else 0,
  ...
  def _add_seq2seq(self):
    ...
    with tf.variable_scope('seq2seq'):
      ...
      with tf.variable_scope('embedding'):
        ...
        emb_enc_inputs_full = tf.nn.embedding_lookup(embedding, self._enc_batch_full) # tensor with shape (batch_size, max_enc_steps, emb_size)
        emb_dec_inputs_full = [tf.nn.embedding_lookup(embedding, x) for x in tf.unstack(self._dec_batch_full, axis=1)] # list length max_dec_steps containing shape (batch_size, emb_size)
        if hps.rl_training:
          emb_enc_inputs_partial = tf.nn.embedding_lookup(embedding, self._enc_batch_partial) # tensor with shape (batch_size, max_enc_steps, emb_size)
          emb_dec_inputs_partial = [tf.nn.embedding_lookup(embedding, x) for x in tf.unstack(self._dec_batch_partial, axis=1)] # list length max_dec_steps containing shape (batch_size, emb_size)

      # Add the encoder.
      enc_outputs_full, fw_st_full, bw_st_full = self._add_encoder(emb_enc_inputs_full, self._enc_lens_full)
      self._enc_states_full = enc_outputs_full
      if hps.rl_training:
        enc_outputs_partial, fw_st_partial, bw_st_partial = self._add_encoder(emb_enc_inputs_partial, self._enc_lens_partial, partial=True)
        self._enc_states_partial = enc_outputs_partial

      # Our encoder is bidirectional and our decoder is unidirectional so we need to reduce the final encoder hidden state to the right size to be the initial decoder hidden state
      self._dec_in_state_full = self._reduce_states(fw_st_full, bw_st_full)
      if hps.rl_training:
        self._dec_in_state_partial = self._reduce_states(fw_st_partial, bw_st_partial, partial=True)
      _t = time.time()
      # Add the decoder.
      with tf.variable_scope('decoder') as scope:
        (
        self.decoder_outputs_full, self._dec_out_state_full, self.attn_dists_full, self.p_gens_full, self.coverage_full,
        self.vocab_scores_full,
        self.final_dists_full, self.samples_full, self.greedy_search_samples_full, self.temporal_es_full,
        self.sampling_rewards_full, self.greedy_rewards_full) = self._add_decoder_full(emb_dec_inputs_full, embedding)
        if hps.rl_training:
          scope.reuse_variables()
          (
            self.decoder_outputs_partial, self._dec_out_state_partial, self.attn_dists_partial, self.p_gens_partial,
            self.coverage_partial,
            self.vocab_scores_partial,
            self.final_dists_partial, self.samples_partial, self.greedy_search_samples_partial,
            self.temporal_es_partial,
            self.sampling_rewards_partial, self.greedy_rewards_partial) = self._add_decoder_partial(
            emb_dec_inputs_partial, embedding)
    ...
    if hps.mode == "decode":
      ...
      assert len(self.final_dists_full)==1 # final_dists is a singleton list containing shape (batch_size, extended_vsize)
      self.final_dists_full = self.final_dists_full[0]
      topk_probs_full, self._topk_ids_full = tf.nn.top_k(self.final_dists_full, hps.batch_size*2) # take the k largest probs. note batch_size=beam_size in decode mode
      self._topk_log_probs_full = tf.log(topk_probs_full)
...
'''
def decode_onestep(self, sess, batch, latest_tokens, enc_states, dec_init_states, prev_coverage, prev_decoder_outputs,
                   prev_encoder_es):
  """For beam search decoding. Run the decoder for one step.
  ...
  feed = {
    self._enc_states_full: enc_states,
    self._enc_padding_mask_full: batch.enc_padding_mask,
    self._dec_in_state_full: new_dec_in_state,
    self._dec_batch_full: np.transpose(np.array([latest_tokens])),
    self._dec_padding_mask_full: np.ones((beam_size, 1), dtype=np.float32)
  }

  to_return = {
    "ids": self._topk_ids_full,
    "probs": self._topk_log_probs_full,
    "states": self._dec_out_state_full,
    "attn_dists": self.attn_dists_full,
    "final_dists": self.final_dists_full
  }

  if FLAGS.pointer_gen:
    feed[self._enc_batch_extend_vocab_full] = batch.enc_batch_extend_vocab
    feed[self._max_art_oovs_full] = batch.max_art_oovs
    to_return['p_gens'] = self.p_gens_full

  if self._hps.coverage:
    feed[self.prev_coverage_full] = np.stack(prev_coverage, axis=0)
    to_return['coverage'] = self.coverage_full

  if FLAGS.ac_training or FLAGS.intradecoder:
    to_return['output'] = self.decoder_outputs_full
  if FLAGS.intradecoder:
    feed[self.prev_decoder_outputs_full] = prev_decoder_outputs
  if FLAGS.use_temporal_attention:
    to_return['temporal_e'] = self.temporal_es_full
    feed[self.prev_encoder_es_full] = prev_encoder_es
  ...
  if FLAGS.ac_training or FLAGS.intradecoder:
    output = results['output'][
      0]  # used for calculating the intradecoder at later steps and for calcualting q-estimate in Actor-Critic training.
  else:
    output = None
  if FLAGS.use_temporal_attention:
    temporal_e = results['temporal_e'][0]  # used for calculating the attention at later steps
