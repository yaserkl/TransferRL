...
class BeamSearchDecoder(object):
  ...
  def __init__(self, model, batcher, vocab, dqn = None):
    ...
    if FLAGS.single_pass:
      ...
      ckpt_name = "{}-ckpt-".format(FLAGS.decode_from) + ckpt_path.split('-')[-1] # this is something of the form "ckpt-123456"
...
def get_decode_dir_name(ckpt_name):
  ...
  if "train" in FLAGS.full_data_path or "train" in FLAGS.partial_data_path: dataset = "train"
  elif "val" in FLAGS.full_data_path or "val" in FLAGS.partial_data_path: dataset = "val"
  elif "test" in FLAGS.full_data_path or "test" in FLAGS.partial_data_path: dataset = "test"
  else: raise ValueError("FLAGS.full_data_path or FLAGS.partial_data_path %s should contain one of train, val or test" % (FLAGS.full_data_path))
  full_or_partial = "partial" if FLAGS.partial_decoding else "full"
  dirname = "decode_%s_%s_%s_%imaxenc_%ibeam_%imindec_%imaxdec" % (
  dataset, full_or_partial, FLAGS.decode_name, FLAGS.max_enc_steps, FLAGS.beam_size, FLAGS.min_dec_steps,
  FLAGS.max_dec_steps)
