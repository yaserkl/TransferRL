...
def pre_processing(text):
    ...
    for i,s in enumerate(doc.sents):
        ...
        for word in s:
            ...
            prev_ent_type = word.ent_type_
        line_text.append('{} .'.format(' '.join(line)))
        pos_text.append('{} .'.format(' '.join(pos)))
        ner_text.append('{} .'.format(' '.join(ner)))
    return '\n'.join(line_text), '\n'.join(pos_text), '\n'.join(ner_text)
...
for filetype in filetypes:
    ...
    print("processing {} files from {}...".format(len(entries),filetype))
    ...
    pwriter = open('{}/processed_{}_wPUNK.bin'.format(root_dir,filetype), 'wb')
    rwriter = open('{}/{}_wPUNK.bin'.format(root_dir, filetype), 'wb')
    with jsonl.open('{}/processed_{}_wPUNK.data'.format(root_dir, filetype), gzip = True) as processed_dataset_file:
...
vsizes = [50,100,150]
for _ in vsizes:
    fw = open('{}/vocab-{}k_wPUNK'.format(root_dir,_), 'w')
    for (k,v) in vocab.most_common(_*1000):
        fw.write('{} {}\n'.format(k,v))
    fw.close()

    fw = open('{}/processed_vocab-{}k_wPUNK'.format(root_dir,_), 'w')
    for (k,v) in pvocab.most_common(_*1000):
        fw.write('{} {}\n'.format(k,v))
    fw.close()
